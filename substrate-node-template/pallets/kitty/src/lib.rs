...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type KittyIndex: StorageHasher;

		type MaxKittyReverse: Get<BalanceOf<Self>>;
	...
	pub(super) type Kitties<T: Config> = StorageMap<_, T::KittyIndex, T::Hash, Kitty<T>>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_kitty(origin: OriginFor<T>) -> DispatchResult {
			...
			log::info!("free_balance: {:?},  maxReverse: {:?}", T::Currency::free_balance(&sender), T::MaxKittyReverse::get());
			//确保有足够的余额可以质押
			ensure!(T::Currency::free_balance(&sender) > T::MaxKittyReverse::get(), <Error<T>>::NotEnoughBalance);
			//质押一定数量的Token
			T::Currency::reserve(&sender, T::MaxKittyReverse::get())?;
		...
		pub fn sell_kitty(
			...
			Self::transfer_to(origin, to , kitty_id)
		}
	...
	impl<T: Config> Pallet<T> {
		...
		fn transfer_to(origin: OriginFor<T>, to: T::AccountId, kitty_id: T::Hash) -> DispatchResult{
